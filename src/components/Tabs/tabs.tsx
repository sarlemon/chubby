...
export const Tabs: FC<TabsProps> = (props) => {
  ...
  const navClass = classNames('chubby-tabs-nav', {
    'nav-line': type === 'line',
    'nav-card': type === 'card',
  })
  const renderNavLinks = () => {
    return React.Children.map(children, (child, index) => {
      ...
      const classes = classNames('chubby-tabs-nav-item', {
  ...
  return (
    <div className={`chubby-tabs ${className}`}>
      <ul className={navClass}>
        {renderNavLinks()}
      </ul>
      <div className="chubby-tabs-content">
        {renderContent()}
      </div>
    </div>
  )
